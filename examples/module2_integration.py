...
def display_backtest_results(results):
    ...
    # Header
    print(f"{'Period':<8} {'CAGR':<8} {'Total Ret':<10} {'Max DD':<10} {'Sharpe':<8} {'End Value':<12}")
    print("-" * 80)
    
    # Results for each period
    for period in [BacktestPeriod.ONE_YEAR, BacktestPeriod.FIVE_YEARS, BacktestPeriod.TEN_YEARS]:
...
def analyze_performance(results):
    ...
    # CAGR analysis
    cagrs = {period: result["performance_metrics"]["cagr"] 
    ...
    # Risk analysis
    drawdowns = {period: result["performance_metrics"]["max_drawdown"] 
                for period, result in successful_results.items()}
    
    lowest_dd_period = min(drawdowns, key=drawdowns.get)
    highest_dd_period = max(drawdowns, key=drawdowns.get)
    
    print(f"• Lowest Risk: {lowest_dd_period} ({drawdowns[lowest_dd_period]:.1%} max drawdown)")
    print(f"• Highest Risk: {highest_dd_period} ({drawdowns[highest_dd_period]:.1%} max drawdown)")
    
    # Risk-adjusted returns
    sharpes = {period: result["performance_metrics"]["sharpe_ratio"] 
              for period, result in successful_results.items()}
    
    best_sharpe_period = max(sharpes, key=sharpes.get)
    
    print(f"• Best Risk-Adjusted Return: {best_sharpe_period} (Sharpe: {sharpes[best_sharpe_period]:.2f})")
