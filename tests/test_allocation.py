...
def test_basic_allocation():
    ...
    result = calculate_investment_allocation(
        target_allocation, investment_amount, current_portfolio
    )
...
def test_tie_breaking():
    ...
    target_allocation = {
        ...
        "GOOGL": 0.33,
        "MSFT": 0.33,
    }  # Equal allocations for true ties
    investment_amount = 1000.0
    ...
    assert (
        len(securities_chosen) == 1
    ), "Should have only one security chosen with random tie-breaking"
...
def test_invalid_inputs():
    ...
    security, amount = result
    assert (
        security is None and amount == 0.0
    ), "Should return None, 0.0 for empty target"
    ...
    assert (
        security is None and amount == 0.0
    ), "Should return None, 0.0 for negative investment"
...
def test_balanced_portfolio():
    ...
    assert (
        amount == expected_amounts[security]
    ), f"Expected {expected_amounts[security]} for {security}, got {amount}"
